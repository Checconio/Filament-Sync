...
const loadCustomProfiles = () => {
    ...
    switch (osType) {
        case 'Darwin':
            ...
            crealityProfileDir = homeDir + '/Library/Application Support/Creality/Creality Print/7.0/user/' + USERID + '/filament/base/'
            if (SLICER == 'orca') {
        ...
        case 'Linux':
            ...
            crealityProfileDir = homeDir + '/.config/Creality/Creality Print/7.0/user/' + USERID + '/filament/base/'
            if (SLICER == 'orca') {
                orcaFiles = fs.readdirSync(orcaProfileDir)
                if (orcaFiles[0] == '.DS_Store') orcaFiles.splice(0, 1)
                orcaFiles = orcaFiles.filter(profile => profile.includes('.json'))
        ...
        case 'Windows_NT':
            ...
            crealityProfileDir = homeDir + '/AppData/Roaming/Creality/Creality Print/7.0/user/' + USERID + '/filament/'
            if (SLICER == 'orca') {
                orcaFiles = fs.readdirSync(orcaProfileDir)
                if (orcaFiles[0] == '.DS_Store') orcaFiles.splice(0, 1)
                orcaFiles = orcaFiles.filter(profile => profile.includes('.json'))
